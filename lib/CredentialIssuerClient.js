...
import CredentialIssuerError, {errors} from './errors/CredentialIssuerError';
...
class CredentialIssuerClient {
  ...
  async getIssuerId() {
    try {
      ...
      const {credential_issuer_id} = data;
      if (credential_issuer_id) return credential_issuer_id;
    } catch (error) {
  ...
  async getIssuerVerifier(authorizableAttributeId) {
    try {
      ...
      const {data} = await axios.get(`${this.url}/authorizable_attribute/${authorizableAttributeId}`);
      console.log("Response from credential issuer: ", data);
      const {verification_key} = data;
      if (verification_key) return verification_key;
    } catch (error) {
      throw new CredentialIssuerError(`Error getting credential issuer verifier: ${error}`);
    }
    throw new CredentialIssuerError('Error getting credential issuer verifier: No verification_key returned');
  }

  async issueCredential(authorizableAttributeId, blindSignRequest, verifyingData) {
    let response;
    let values = [];
    try {
      for (const k in verifyingData) {
        values.push({
          name: k,
      ...
      if (status == 412) {
        if (detail === 'Credential already issued') {
