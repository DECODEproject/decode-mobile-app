...
export async function isComingFromDevice() {
  return getInitialUrl().then((rawUrl) => {
    const url = urlParse(rawUrl, true);
    const {
      hostname: action,
      query: { deviceToken, deviceLocation, deviceExposure }
    } = url;
    return action === 'iot' ? {deviceToken, deviceLocation, deviceExposure} : null;
  });
}
