...
export default (goToPetition, goToLogin, goToDevice) => event => {
    ...
    if (action === 'iot') {
      const { query: { deviceToken, deviceLocation, deviceExposure } } = urlObj;
      console.log('Going to device onboarding: ', deviceToken, deviceLocation, deviceExposure);
      goToDevice({deviceToken, deviceLocation, deviceExposure});
      return;
    }
